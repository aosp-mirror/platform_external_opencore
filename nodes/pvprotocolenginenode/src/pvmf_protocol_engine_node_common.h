...
// This class is a factory class to create ProtocolContainer
class ProtocolContainer;
class PVMFProtocolEngineNode;
class ProtocolContainerFactory
{
    public:
        virtual ProtocolContainer* create(PVMFProtocolEngineNode *aNode = NULL) = 0;
};
...
class ProgressiveDownloadContainerFactory : public ProtocolContainerFactory
{
    public:
...
class ProtocolContainer
{
    public:
        ...
        virtual bool createProtocolObjects() = 0;
...
class PVMFProtocolEngineNodeOutput
{
    public:
        ...
        virtual void setOutputObject(OsclAny* aOutputObject, const uint32 aObjectType = NodeOutputType_InputPortForData);
        ...
        virtual ~PVMFProtocolEngineNodeOutput();

    protected:
        // for media data
        PVMFStatus createMemPool();
        void deleteMemPool();
        // create media data for composing http request
        bool createMediaData(PVMFSharedMediaDataPtr &aMediaData, uint32 aRequestSize);
        bool sendToDestPort(PVMFSharedMediaDataPtr &aMediaData, PVMFProtocolEnginePort *aPort);

        // send the input media data to the port
        virtual bool sendToPort(PVMFSharedMediaDataPtr &aMediaData, const uint32 aPortType = NodeOutputType_InputPortForData);
...
class UserAgentField
{
    public:
        ...
        UserAgentField(OSCL_wString &aUserAgent, const bool isOverwritable = false);
        UserAgentField(OSCL_String &aUserAgent, const bool isOverwritable = false);
        ...
        bool setUserAgent(OSCL_String &aUserAgent, const bool isOverwritable = false);
        ...
        bool getUserAgent(OSCL_String &aUserAgent);
...
class EventReporter
{
    public:
        ...
        // constructor
        EventReporter(PVMFProtocolEngineNode *aNode);
...
class InterfacingObjectContainer
{
    public:
        ...
        uint32 setHttpHeader(OUTPUT_DATA_QUEUE &aHttpHeader);
        ...
        // Streaming proxy
        bool setStreamingProxy(OSCL_wString& aProxyName, const uint32 aProxyPort = DEFAULT_HTTP_PORT_NUMBER);
...
class PVMFProtocolEngineNodeTimer
{
    public:
        ...
        static PVMFProtocolEngineNodeTimer* create(OsclTimerObserver *aObserver);
        ~PVMFProtocolEngineNodeTimer();

        // register and set timer id and timeout value
        // note that if aTimeout=0, that means using the internal default one
        void set(const uint32 aTimerID, const int32 aTimeout = 0);
        ...
        uint32 getTimeout(const uint32 aTimerID);
        ...
        bool start(const uint32 aTimerID, const int32 aTimeout = 0);

        void cancel(const uint32 aTimerID);
        void clear();

        // clear all the timers except the timer with the given timer ID
        void clearExcept(const uint32 aTimerID);
